...
// StatsPage renders the stats query HTML page.
func (h *Handlers) StatsPage(w http.ResponseWriter, _ *http.Request) {
	inventories := h.manager.List()

	// Filter to only loaded inventories
	...
	if err := h.renderer.Render(w, "stats.html", data); err != nil {
		h.logger.Error().Err(err).Msg("failed to render stats page")
...
func (h *Handlers) StatsResultPartial(w http.ResponseWriter, r *http.Request) {
	...
	w.Header().Set("Content-Type", "text/html; charset=utf-8")
	if err := h.renderer.RenderPartial(w, "stats_result.html", resp); err != nil {
		h.logger.Error().Err(err).Msg("failed to render stats result")
